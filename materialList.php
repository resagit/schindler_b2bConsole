...
?>
  ...
  //  create_me(hd);
    ...
    function load()
    {
        $("#tbd1").html("<tr><td colspan='6' style='text-align:center;'>No Records Found</td></tr>");
        ...
        if(a == 'N'){
            ...
            for(var i = 0; i < obj.LIST.length; i++)
            {
                ...
                if(datakk[i][3]==null || datakk[i][3]=="") {
                    $("#tbd1").append("<tr><td class='item_image'><img src='no-image.jpg' style='width:80px;height:80px; border:1px solid #000;'></td><td class='item_code'>" + datakk[i][0] + "</td><td class='material_name'>" + datakk[i][1] + "</td><td class='baseuom'> " + datakk[i][2] + "</td><td><button class='btn btn-default' onclick='set_item_id(this.title)' title='"+datakk[i][0]+"'><i class='fa fa-upload'></i> Add Image</button></td><td><button class='btn btn-default' onclick='edit_items(this.title)' title='"+datakk[i][4]+"'><i class='fa fa-edit'></i> Edit Material</button></td></tr>");
                }
                else {
                    $("#tbd1").append("<tr><td class='item_image'><img class='img-viewer' title='Click To View Full' src='" + datakk[i][3] +"' style='width:80px;height:80px; border:1px solid #000;'></td><td class='item_code'>" + datakk[i][0] + "</td><td class='material_name'>" + datakk[i][1] + "</td><td class='baseuom'> " + datakk[i][2] + "</td><td><button class='btn btn-default'  onclick='set_item_id(this.title)' title='"+datakk[i][0]+"'><i class='fa fa-upload'></i> Change Image</button></td><td><button class='btn btn-default'  onclick='edit_items(this.title)' title='"+datakk[i][4]+"'><i class='fa fa-edit'></i> Edit Material</button></td></tr>");
                }
            }
        ...
        $.get("getMaterialDetailsForEdit.php",cmt_id, function (tt) {
         ...
         //alert(tt+"edit");
            ...
            for(var i = 0; i < obj.LIST.length; i++)
            {
                ...
                edit_item[i][15]=obj.LIST[i].serialNoStatus;
            ...
            fields: {
                ...
                , material_name: {
                    message: 'The Material Name is not valid', validators: {
                        ...
                        , stringLength: {
                            min: 2, max: 80, message: 'The Material Name  be more than 2 and less than 80 characters long'
                        }
                    }
                }
                , base_uom: {
                ...
                , material_order_quantity: {
                    message: 'Material Order Quantity is not valid', validators: {
                        notEmpty: {
                            message: 'Material Order Quantity is required and cannot be empty'
                        }
                        , regexp: {
                            regexp: /^[0-9.]+$/i, message: 'Material Order Quantity  cannot be character'
                        }
                    }
                }
            }
