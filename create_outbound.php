...
//alert(levelOptions1);
            ...
            function material_stock()
            {
                if($("#autocomplete").val()!="") {
                    var warehouse_id = $("#warehouse_id").val();
                    var material_id = $("#autocomplete").val();
                    var sales_id = $("#ids_value").val();
                    var quers = {"warehouse_id": warehouse_id, "material_id": material_id, "sales_id": sales_id};
                    $('#stock').html("<img style='height: 200px;' src='loading_satus.gif'>");
                    $.post("CurrentStockForMaterial.php", quers, function (fd) {


//alert(fd);//{"Stock":[{"currentStock":-16934,"unitOfMeasurement":"piece"}]}

                        if ($.trim(fd) == 'N') {
                            $('#stock').html("Current Stock for " + material_id + " is : not available at selected warehouse")
                        } else {
                            var obj = JSON.parse(fd);
                            ...
                            for (var i = 0; i < obj.Stock.length; i++) {
                                ...
                                uom11[i] = new Array();
                                uom11[i][0] = obj.Stock[i].currentStock;
                                uom11[i][1] = obj.Stock[i].unitOfMeasurement;
                                /*data1[i][2]=obj.UOM[i].tinNo;
                                 data1[i][3]=obj.UOM[i].cinNo;
                                 data1[i][4]=obj.UOM[i].id;*/
                                //data[i][1]=obj.LIST[i].name;
...
//$("#mes").html("Current Stock for "+material_id+" is :"+uom11[i][0]+" in "+uom11[i][1]+"  at selected warehouse" );

                                //   show_modal("Current Stock for "+material_id+" is : "+uom11[i][0]+" in "+uom11[i][1]+"  at selected warehouse");
                                $('#stock').html("Current Stock for " + material_id + " is : " + uom11[i][0] + " in " + uom11[i][1] + "  at selected warehouse")
                            }
                        }
                    });
                }else {$('#stock').html("")}
