...
$new_matri=str_replace("~","*",$materialItemID);
...
$shipperRefName=urlencode(str_replace("/","`",$_POST['shipperRefName']));
$shipperRefMobileNo=$_POST['shipperRefMobileNo'];
$consigneeRefName=urlencode(str_replace("/","`",$_POST['consigneeRefName']));
...
$chalanNo=strtoupper($_POST['chalanNO']);
$chalanDate=$_POST['chalanDate'];
$materialItemID=str_replace("~","*",$_POST['materialItemID']);
...
$perUnitPrice=check_value($perUnitPrice);
                     ...
                     //createOrderWithPdf/{companyId}/{coID}/{consigneeID}/{chalanNo}/{chalanDate}/{materialItemID}/{quantity}/{uom}/{orderType}/{shipperRefName}/{shipperRefMobileNo}/{consigneeRefName}/{consigneeRefMobileNo}/{perUnitPrice}/{specialInstruction}/{userName}/{userType}/{wareHouseID}/{shipperID}/{fromId}/{toWareHouseID}/{toCompanySalesOfficeID}
$url="http://".$ip."/createOrderWithPdf/";
//$url="http://".$ip."/ResagitB2B/createOrderWithPdf/$sales_off/$coID/$consigneeID/$chalanNo/$chalanDate/$materialItemID/$quantity/$uom/$orderType/$shipperRefName/$shipperRefMobileNo/$consigneeRefName/$consigneeRefMobileNo/$perUnitPrice/$instruction/$username/$userType/$warehouse_id/$shipper_id/$cnf_from_id/$to_warehouse/$to_company";
/*echo $username;*/
//echo $url;
$ch = curl_init();

$data1 = array('customerFile' => "@".getcwd()."/files/$res1",'companyFile'=>"@".getcwd()."/files/$res","companyId"=>$sales_off,"coID"=>$coID,"consigneeID"=>$consigneeID,"chalanNo"=>$chalanNo,"chalanDate"=>$chalanDate,"materialItemID"=>$new_matri,"quantity"=>$quantity,"uom"=>$uom,"orderType"=>$orderType,"shipperRefName"=>$shipperRefName,"shipperRefMobileNo"=>$shipperRefMobileNo,"consigneeRefName"=>$consignee_name,"consigneeRefMobileNo"=>$consigneeRefMobileNo,"perUnitPrice"=>$perUnitPrice,"specialInstruction"=>$instruction,"userName"=>$username,"userType"=>$userType,"wareHouseID"=>$warehouse_id,"shipperID"=>$shipper_id,"fromId"=>$cnf_from_id,"toWareHouseID"=>$to_warehouse,"toCompanySalesOfficeID"=>$to_company,"bomId"=>$bom_id,"bomQuantity"=>$bom_qty);
...
{
    echo("<script>console.log('PHP: ".$data."');</script>");
    echo "<label><i class=\"fa fa-exclamation-circle fa-2x\"></i> &nbsp;&nbsp; ".ucwords($data)."</label>";
